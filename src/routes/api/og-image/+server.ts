...
export const GET: RequestHandler = async ({ url, fetch }) => {
	...
	try {
		...
		const logoBgSize = LOGO_SIZE + BG_PADDING * 2;
		const logoBg = await sharp({
			...
			.composite([
				{
					input: await sharp(Buffer.from(LOGO_SVG)).resize(LOGO_SIZE, LOGO_SIZE).png().toBuffer(),
					gravity: 'centre'
				}
			])
			.toBuffer();
		...
		const result = await sharp(coverBuffer)
			...
			.composite([
				{
					input: logoBg,
					top: CANVAS_SIZE - logoBgSize - LOGO_PADDING,
					left: CANVAS_SIZE - logoBgSize - LOGO_PADDING
				}
			])
			.jpeg({ quality: 90 })
			.toBuffer();
