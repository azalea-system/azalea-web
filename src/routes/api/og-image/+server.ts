import type { RequestHandler } from './$types';
import sharp from 'sharp';

const CANVAS_SIZE = 600;
const OVERLAY_SIZE = 140;
const OVERLAY_PADDING = 30;

const DEFAULT_OVERLAY_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="632" height="562" viewBox="-28 -25 632 562"><rect x="-28" y="-25" width="632" height="562" rx="40" fill="rgba(0,0,0,0.7)"/><path fill="#2e76ff" d="M288 0c17.7 0 32 14.3 32 32v17.7C451.8 63.4 557.7 161 573.9 285.9c2 15.6-17.3 24.4-27.8 12.7c-14-15.6-41.3-26.6-66.1-26.6c-38.7 0-71 27.5-78.4 64.1c-1.7 8.7-8.7 15.9-17.6 15.9s-15.8-7.2-17.6-15.9C359 299.5 326.7 272 288 272s-71 27.5-78.4 64.1c-1.7 8.7-8.7 15.9-17.6 15.9s-15.8-7.2-17.6-15.9C167 299.5 134.7 272 96 272c-24.8 0-52.1 11-66.1 26.7c-10.5 11.7-29.8 2.8-27.8-12.8C18.3 161 124.2 63.4 256 49.7V32c0-17.7 14.3-32 32-32m0 304c12.3 0 23.5 4.6 32 12.2v114.3c0 45-36.5 81.4-81.4 81.4c-30.8 0-59-17.4-72.8-45l-2.3-4.7c-7.9-15.8-1.5-35 14.3-42.9s35-1.5 42.9 14.3l2.3 4.7c3 5.9 9 9.6 15.6 9.6c9.6 0 17.4-7.8 17.4-17.4V316.2c8.5-7.6 19.7-12.2 32-12.2"/></svg>`;

export const GET: RequestHandler = async ({ url, fetch }) => {
	const mbid = url.searchParams.get('mbid');
	const overlaySvg = url.searchParams.get('overlay') ?? DEFAULT_OVERLAY_SVG;

	if (!mbid) {
		return new Response('Missing mbid parameter', { status: 400 });
	}

	const coverArtUrl = `https://coverartarchive.org/release-group/${mbid}/front`;

	try {
		const response = await fetch(coverArtUrl);
		if (!response.ok) {
			return new Response('Failed to fetch cover art', { status: 502 });
		}
		const coverBuffer = Buffer.from(await response.arrayBuffer());

		const overlayBuffer = await sharp(Buffer.from(overlaySvg))
			.resize(OVERLAY_SIZE, OVERLAY_SIZE)
			.png()
			.toBuffer();

		const result = await sharp(coverBuffer)
			.resize(CANVAS_SIZE, CANVAS_SIZE, { fit: 'cover' })
			.composite([
				{
					input: overlayBuffer,
					top: CANVAS_SIZE - OVERLAY_SIZE - OVERLAY_PADDING,
					left: CANVAS_SIZE - OVERLAY_SIZE - OVERLAY_PADDING
				}
			])
			.jpeg({ quality: 90 })
			.toBuffer();

		return new Response(result, {
			headers: {
				'Content-Type': 'image/jpeg',
				'Cache-Control': 'public, max-age=86400'
			}
		});
	} catch {
		return new Response('Failed to generate image', { status: 500 });
	}
};
